...
def apply(request):
    ...
    if request.method=='POST':
        ...
        if patientForm.is_valid():
            ...
            patient.assignedId=request.POST.get('assignedDoctorId')
...
def view(request):
    patients=models.Score.objects.all().filter(status=False,assignedId=request.user.id)
    hint = models.Score.objects.all().filter(user__id=request.user.id)
    print(hint)
    ...
    return render(request,'view-feedback.html')